...
Currently, GUI verification is primarily done through the programmatic API and keyboard shortcuts. Automated scripts for agent workflows are under development.
...
# 5. Trigger screenshot manually (F12)
# Or use the programmatic API in tests/automation

# 6. Attach screenshot to PR
# Manually attach the screenshot from the 'screenshots/' directory to the PR.
...
```rust
use rust_daq_app::gui::Gui; // Updated path

// In your test or automation code
gui.request_screenshot("tests/output/test_screenshot.png");
```

See `crates/rust-daq-app/src/gui/verification.rs` for the full verification framework API.
