...
The knowledge graph is accessible from any device on the Tailscale network. For details on Tailscale integration, see [Tailscale Integration Guide](./TAILSCALE_INTEGRATION.md).
...
# Verify Tailscale connectivity (see Tailscale Integration Guide for details)
