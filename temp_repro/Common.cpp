...
bool GetMenuSelection(const std::string& title, const NVPC& menu, int32& selection)
{
    ...
    std::string input;
    if (!WaitForInputString(input))
        return false; // Aborted by user

    if (!StrToNumber<int32>(input, selection))
        return false;
    for (const auto& item : menu)
    {
...
bool WaitForInputString(std::string& str)
{
    std::cin.clear();
    getline(std::cin, str);
    const bool abortedByUser = std::cin.fail();
    if (abortedByUser)
        printf("\n");
...
{
    std::string tmpStr;
    return WaitForInputString(tmpStr);
}
...
{
    printf("\n>>>\n");
    printf(">>> CLI TERMINATION HANDLER\n");
...
{
    const bool retVal = InstallCliTerminationHandler(
            std::bind(GenericCliTerminationHandler, std::cref(contexts)));
    if (retVal)
    {
        printf("Installed CLI termination handler\n");
    }
    return retVal;
}
...
bool ShowAppInfo(int argc, char* argv[])
{
    ...
    // Install no-op handler to disallow app abort on ctr+c
    auto noopCliHandler = []() {
        printf("\n>>>\n");
        printf(">>> CLI TERMINATION HANDLER\n");
        printf(">>> Aborting user input\n");
        printf(">>>\n\n");
    };
    if (!InstallCliTerminationHandler(noopCliHandler))
    {
        printf("Unable to install default termination handler\n");
        return false;
    }
...
bool OpenCamera(CameraContext* ctx)
{
    ...
    if (PV_OK != pl_get_param(ctx->hcam, PARAM_SER_SIZE, ATTR_CURRENT,
                (void*)&ctx->sensorResX))
    {
        PrintErrorMessage(pl_error_code(), "Couldn't read sensor X-resolution");
        return false;
    }
    ...
    if (PV_OK != pl_get_param(ctx->hcam, PARAM_PAR_SIZE, ATTR_CURRENT,
                (void*)&ctx->sensorResY))
    {
        PrintErrorMessage(pl_error_code(), "Couldn't read sensor Y-resolution");
        return false;
    }
    printf("  Sensor size: %ux%u px\n", ctx->sensorResX, ctx->sensorResY);
    ...
    for (const auto& port : ctx->speedTable)
    {
        ...
        {
            printf("    - speed index %d, name: '%s'\n",
                    speed.index, speed.name.c_str());
...
bool InitAndOpenMultipleCameras(std::vector<CameraContext*>& contexts, uns16& camCount)
{
    ...
    {
        printf("Detected more cameras than configured for this sample. Only %u cameras will be used.\n\n",
                nrOfCameras);
    }
    else if (contexts.size() < camCount)
    {
        printf("Detected fewer cameras than configured for this sample. Only %u cameras will be used.\n\n",
                nrOfCameras);
    }
...
bool GetSpeedTable(const CameraContext* ctx, std::vector<SpdtabPort>& speedTable)
{
    ...
    rs_bool isSpeedNameAvailable;
    if (PV_OK != pl_get_param(ctx->hcam, PARAM_SPDTAB_NAME, ATTR_AVAIL,
        (void*)&isSpeedNameAvailable))
    {
        printf("Error reading ATTR_AVAIL of PARAM_SPDTAB_NAME\n");
        return false;
    }
    const bool isSpeedNameSupported = isSpeedNameAvailable != FALSE;
    ...
    for (size_t pi = 0; pi < ports.size(); pi++)
    {
        ...
        for (int16 si = 0; si < (int16)speedCount; si++)
        {
            ...
            // Read the speed name, if supported. Use 'MHz' notation by default.
            std::string speedName = std::to_string(1000.0 / pixTime) + " MHz";
            if (isSpeedNameSupported)
            {
                char speedNameBuf[MAX_SPDTAB_NAME_LEN];
                if (PV_OK != pl_get_param(ctx->hcam, PARAM_SPDTAB_NAME,
                    ATTR_CURRENT, (void*)speedNameBuf))
                {
                    PrintErrorMessage(pl_error_code(),
                        "pl_get_param(PARAM_SPDTAB_NAME) error");
                    return false;
                }
                speedName = speedNameBuf;
            }
...
std::string BufferToStrT(const T* pBuffer, uns32 bufferBytes, uns32 maxItems)
{
    ...
    {
        // The unary plus operator is necessary to print also uns8 as number, not as char
        ss << " " << +pBuffer[n];
    }
    return ss.str();
}
        ...
        uns32 bufferBytes, uns32 maxItems = 5)
{
    ...
    case PL_IMAGE_FORMAT_BAYER8:
        return BufferToStrT(reinterpret_cast<const uns8*>(pBuffer), bufferBytes, maxItems);
    case PL_IMAGE_FORMAT_MONO16:
    case PL_IMAGE_FORMAT_BAYER16:
        return BufferToStrT(reinterpret_cast<const uns16*>(pBuffer), bufferBytes, maxItems);
    case PL_IMAGE_FORMAT_MONO32:
    case PL_IMAGE_FORMAT_BAYER32:
        return BufferToStrT(reinterpret_cast<const uns32*>(pBuffer), bufferBytes, maxItems);
    default:
        return " <unsupported image format>";
    }
}

void PrintImageData(const CameraContext* ctx, const void* pBuffer,
        uns32 bufferBytes, const char* title)
{
    std::string subTitle;
...
void PrintMetaExtMd(void* pMetaData, uns32 metaDataSize)
{
    ...
    for (uns16 i = 0; i < extMdCol.count; ++i)
    {
...
void PrintMetaRoi(const CameraContext* ctx, const md_frame* pFrameDesc, uns16 roiIndex)
{
    const md_frame_header* pFrameHdr = pFrameDesc->header;
    const md_frame_roi* pRoiDesc = &pFrameDesc->roiArray[roiIndex];
    ...
    printf("================================== ROI HEADER =================================\n");
    printf(" roiNr:%u, timestampBOR:%u, timestampEOR:%u\n",
            pRoiHdr->roiNr, pRoiHdr->timestampBOR, pRoiHdr->timestampEOR);
    printf(" roi:[%u,%u,%u,%u,%u,%u], flags:0x%02x, extendedMdSize:%u\n",
            roi.s1, roi.s2, roi.sbin, roi.p1, roi.p2, roi.pbin,
            pRoiHdr->flags, pRoiHdr->extendedMdSize);
    if (pFrameHdr->version >= 2)
    {
        printf(" roiDataSize:%u\n",
                pRoiHdr->roiDataSize);
    }
        ...
        bool printAllRois)
{
    ...
    printf(" roiCount:%u, roiCapacity:%u, impliedRoi:[%u,%u,%u,%u,%u,%u]\n",
            pFrameDesc->roiCount, pFrameDesc->roiCapacity,
            impRoi.s1, impRoi.s2, impRoi.sbin, impRoi.p1, impRoi.p2, impRoi.pbin);
    printf("================================= FRAME HEADER ================================\n");
    printf(" version:%u, frameNr:%u, roiCount:%u\n",
            pFrameHdr->version, pFrameHdr->frameNr, pFrameHdr->roiCount);
    if (pFrameHdr->version < 3)
    {
        printf(" timestampBOF:%u, timestampEOF:%u, timestampResNs:%u\n",
                pFrameHdr->timestampBOF, pFrameHdr->timestampEOF,
                pFrameHdr->timestampResNs);
        printf(" exposureTime:%u, exposureTimeResNs:%u, roiTimestampResNs:%u\n",
                pFrameHdr->exposureTime, pFrameHdr->exposureTimeResNs,
                pFrameHdr->roiTimestampResNs);
    }
    else
    {
        printf(" timestampBOF:%llu, timestampEOF:%llu, exposureTime:%llu\n",
                (unsigned long long)pFrameHdr->timestampBOF,
                (unsigned long long)pFrameHdr->timestampEOF,
                (unsigned long long)pFrameHdr->exposureTime);
    }
    printf(" bitDepth:%u, colorMask:%u, flags:0x%02x, extendedMdSz:%u\n",
            pFrameHdr->bitDepth, pFrameHdr->colorMask, pFrameHdr->flags,
            pFrameHdr->extendedMdSize);
    if (pFrameHdr->version >= 2)
    {
        printf(" imageFormat:%u, imageCompression:%u\n",
                pFrameHdr->imageFormat, pFrameHdr->imageCompression);
    }
    ...
    if (printAllRois)
    {
        for (uns16 i = 0; i < pFrameDesc->roiCount; ++i)
        {
            PrintMetaRoi(ctx, pFrameDesc, i);
        }
    }
    else
    {
        PrintMetaRoi(ctx, pFrameDesc, 0);
    }
    printf("===============================================================================\n");
}
        ...
        const uns32* pExposures, uns16 exposuresCount)
{
    ...
    // number of exposures.
    // The maximum number of exposures is retrieved from the 'entries' field in the
    // smart streaming structure when querying the camera for the ATTR_MAX attribute.
...
std::vector<PvcamPpFeature> DiscoverCameraPostProcessing(CameraContext* ctx)
{
    ...
    if (PV_OK != pl_get_param(ctx->hcam, PARAM_PP_INDEX, ATTR_COUNT, &ppFeatureCount))
    {
        PrintErrorMessage(pl_error_code(),
    ...
    for (int16 ppFeatureIndex = 0; ppFeatureIndex < (int16)ppFeatureCount; ppFeatureIndex++)
    {
        PvcamPpFeature ppFeature; // Store all the information about this feature here
        ...
        uns32 ppFeatureID;
        if (PV_OK != pl_get_param(ctx->hcam, PARAM_PP_FEAT_ID, ATTR_CURRENT,
                    &ppFeatureID))
        {
        ...
        for (int16 ppParamIndex = 0; ppParamIndex < (int16)ppParamCount; ppParamIndex++)
        {
            PvcamPpParameter ppParam; // Store all the information about this parameter here

            // Set index to a particular post processing parameter
            if (PV_OK != pl_set_param(ctx->hcam, PARAM_PP_PARAM_INDEX, &ppParamIndex))
            {
                PrintErrorMessage(pl_error_code(),
            ...
            uns32 minValue;
            if (PV_OK != pl_get_param(ctx->hcam, PARAM_PP_PARAM, ATTR_MIN, &minValue))
            {
                PrintErrorMessage(pl_error_code(),
            ...
            uns32 maxValue;
            if (PV_OK != pl_get_param(ctx->hcam, PARAM_PP_PARAM, ATTR_MAX, &maxValue))
            {
                PrintErrorMessage(pl_error_code(),
            ...
            uns32 defValue;
            if (PV_OK != pl_get_param(ctx->hcam, PARAM_PP_PARAM, ATTR_DEFAULT, &defValue))
            {
                PrintErrorMessage(pl_error_code(),
...
{
    // Tell the camera we want to work with the feature and parameter on given index
    // Set the feature index first
    ...
    {
        PrintErrorMessage(pl_error_code(), "pl_set_param(PARAM_PP_PARAM_INDEX) error");
        return false;
    }
