...
struct SpdtabSpeed
{
    ...
    uns16 pixTimeNs{ 1 };
    // Speed name. Either a 'MHz' notation based on pix time or an actual name,
    // if supported.
    std::string name;
...
struct CameraContext
{
    ...
    Event eofEvent{};
    // The counter of EOF events (frame counter)
    unsigned int eofCounter{ 0 };
...
template<typename T>
bool StrToNumber(const std::string& str,
        typename std::enable_if<std::is_same<T, float>::value, T>::type& number)
{
    try {
        size_t idx;
...
template<typename T>
bool StrToNumber(const std::string& str,
        typename std::enable_if<std::is_same<T, double>::value, T>::type& number)
{
    try {
        size_t idx;
...
// When user presses Enter or provides an invalid value, the function returns false.
...
// a fail flag and this function returns false.
// Use std::cin.fail() check when this function is used together with other
// processing of standard input.
bool WaitForInputString(std::string& str);

// Reads a number from standard input via WaitForInputString function.
// This function returns false only when ctrl+c is pressed.
// If whole input string is not a number or if the number falls outside given
// range, user is asked to retry.
template<typename T>
bool WaitForInputNumber(T min, T max, T def, T& number)
{
    ...
    for (;;)
    {
        std::cout << "Enter number [" << min << " - " << max
            << "] or hit <Enter> for default (" << def << "): ";
        std::string numStr;
        if (!WaitForInputString(numStr))
            return false; // Aborted by user
        if (numStr.length() == 0)
        {
            num = def; // If zero length is received the user simply pressed <Enter>
            std::cout << def << std::endl;
            break;
        }
        if (!StrToNumber<T>(numStr, num))
        {
...
// Reads the standard input via WaitForInputString but ignores the string.
bool WaitForEnter();
...
// Installs custom termination handler function that handles ctrl+c and other terminal
// events. In most cases, the GenericCliTerminationHandler() function can be used.
...
// A helper function that installs the GenericCliTerminationHandler as a default
// termination handler.
...
// Retrieves the last PVCAM error code and displays a PVCAM error message together
...
// If there are fewer than camCount cameras available, the camCount value is
// updated to the number of currently open cameras.
...
// If Smart Streaming is supported, enable it and load given exposures to the camera.
