...
#### 1. GUI Screenshot Functionality (`src/gui/mod.rs`)
...
#### 2. Verification Framework (`src/gui/verification.rs`)
...
#### 3. Python Verification Script (`jules-scratch/verification/verify_gui_screenshot.py`)

Automated verification script for agent workflows:

- **Check application**: Verifies rust-daq is running (`pgrep`)
- **Trigger screenshot**: Simulates F12 keypress via `pyautogui`
- **Verify capture**: Checks screenshot file exists and is recent
- **Copy for PR**: Places screenshot at standard location for documentation
...
- **Screenshot capabilities**: Keyboard shortcut, programmatic API, verification scripts
- **Example workflow**: Step-by-step GUI issue verification with screenshots
- **Comparison**: Playwright vs native verification approaches
...
- Referenced verification scripts and API

#### .gitignore

Added `/screenshots/` directory to prevent auto-generated screenshots from being committed.

#### jules-scratch/verification/README.md

Created comprehensive guide for verification scripts:
...
```python
# jules-scratch/verification/verify_consolidation_toggle.py
...
```python
# jules-scratch/verification/verify_gui_screenshot.py
import pyautogui
import subprocess
...
- Keyboard simulation for interaction
...
# - Triggers screenshot: F12 key

# 4. Verification script confirms success
python jules-scratch/verification/verify_gui_screenshot.py
# Output: ✓ Screenshot verified: screenshots/screenshot_20251023_143522.png

# 5. PR includes visual evidence
gh pr create --body "![Spectrum Plot](./jules-scratch/verification/verification.png)"
...
│       ├── verify_gui_screenshot.py       # New native verification
│       ├── verify_consolidation_toggle.py # Original Playwright
│       └── verification.png        # Standardized location for PRs
└── src/
    └── gui/
