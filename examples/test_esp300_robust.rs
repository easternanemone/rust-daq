...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_configs = vec![
        ...
        ("/dev/ttyUSB1", 19200, serialport::FlowControl::Hardware, "USB1 @ 19200 RTS/CTS"),
        ("/dev/ttyUSB1", 19200, serialport::FlowControl::None, "USB1 @ 19200 No FC"),
        ("/dev/ttyUSB2", 19200, serialport::FlowControl::Hardware, "USB2 @ 19200 RTS/CTS"),
        ("/dev/ttyUSB3", 19200, serialport::FlowControl::Hardware, "USB3 @ 19200 RTS/CTS"),
        ("/dev/ttyUSB1", 9600, serialport::FlowControl::Hardware, "USB1 @ 9600 RTS/CTS"),
        ("/dev/ttyUSB1", 9600, serialport::FlowControl::None, "USB1 @ 9600 No FC"),
    ];
    ...
    for (port_name, baud_rate, flow_control, desc) in &test_configs {
        ...
        match port.read(&mut buffer) {
            Ok(n) if n > 0 => {
                ...
                if trimmed.contains("ESP300") || trimmed.contains("ESP") || trimmed.contains("Newport") {
