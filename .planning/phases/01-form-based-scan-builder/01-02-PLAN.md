...
fn handle_document(&mut self, doc: Document) {
    ...
    match doc.payload {
        ...
        Some(Payload::Event(event)) => {
            ...
            // Extract data for plotting - handled in Task 3
...
- Abort cleans up subscription task
  ...
  <done>Document streaming active during execution, progress bar shows point count and ETA</done>
</task>

<task type="auto">
  <name>Task 3: Add live 1D plot with egui_plot</name>
  <files>crates/daq-egui/src/panels/scan_builder.rs</files>
  <action>
**Add plot data state:**
```rust
...
show_plot: bool,
plot_style: PlotStyle,  // LineWithMarkers, ScatterOnly
...
fn process_event_for_plot(&mut self, event: &daq_proto::daq::EventDoc) {
    ...
    let actuator_pos = self.selected_actuator.as_ref()
        ...
        .copied()
        .unwrap_or(event.seq_num as f64);  // Fallback to sequence number
...
```rust
fn render_live_plot(&self, ui: &mut egui::Ui) {
            ...
            for (idx, (detector_id, points)) in self.plot_data.iter().enumerate() {
                ...
                match self.plot_style {
                    PlotStyle::LineWithMarkers => {
                        plot_ui.line(
                            Line::new(detector_id.clone(), plot_points.clone())
                                .color(color)
                                .width(2.0)
                        );
                        plot_ui.points(
                            Points::new(detector_id.clone(), plot_points)
                                .color(color)
                                .radius(3.0)
                        );
                    }
...
**Clear plot on new scan start:**
In the `StartScan` action setup:
```rust
self.plot_data.clear();
```

**Request repaint during execution:**
At end of `poll_documents()`:
```rust
