...
async fn liveimage_200_frames() {
    ...
    println!("      Sensor: {}x{} ({:.2} MP)", width, height, 
    ...
    while frames_received < target_frames {
        match tokio::time::timeout(Duration::from_secs(30), rx.recv()).await {
            ...
            Ok(Err(tokio::sync::broadcast::error::RecvError::Lagged(n))) => {
                ...
                println!("      [Note: {} frames lagged, total now: {}]", n, frames_received);
            }
            Ok(Err(e)) => {
                panic!("Channel error at frame {}: {}", frames_received, e);
            }
            Err(_) => {
                panic!("TIMEOUT: Only received {} of {} frames", frames_received, target_frames);
            }
        }
    }
    ...
    println!("║                         RESULTS                              ║");
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║  Frames received:  {:4}                                      ║", frames_received);
    println!("║  Last frame #:     {:4}                                      ║", last_frame_num);
    println!("║  Total time:       {:6.2}s                                   ║", elapsed.as_secs_f64());
    println!("║  Frame rate:       {:6.2} fps                                ║", 
    ...
    if let Some(data) = first_frame_data {
        ...
        println!("\n  ✓ Data verified as REAL camera data (max={} << 4096)", max);
    }
    
    // Final assertions
    assert!(frames_received >= target_frames, 
        "Should receive at least {} frames, got {}", target_frames, frames_received);
