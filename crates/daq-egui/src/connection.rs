...
//! 2. Persisted from previous session (via eframe::Storage)
...
//!
//! ```
//! use daq_egui::connection::{DaemonAddress, AddressSource};
//!
...
//! assert!(!addr.is_tls());
//! # Ok::<(), daq_egui::connection::AddressError>(())
//! ```
...
pub enum AddressSource {
    ...
    Environment,
    /// Restored from previous session via eframe::Storage
    Persisted,
...
///
/// ```
/// use daq_egui::connection::normalize_url;
///
/// // Bare host:port
...
/// 2. Persisted from eframe::Storage
/// 3. `DAQ_DAEMON_URL` environment variable
/// 4. Default: `http://127.0.0.1:50051`
///
/// # Arguments
...
/// * `storage` - Optional eframe::Storage for loading persisted address
///
/// # Returns
///
...
pub fn resolve_address(
    ...
    if let Some(storage) = storage {
        if let Some(persisted) = storage.get_string(STORAGE_KEY_DAEMON_ADDR) {
...
///
/// Only the URL string is saved (not the source), as the source will
/// be set to `Persisted` when loading.
pub fn save_to_storage(storage: &mut dyn eframe::Storage, address: &DaemonAddress) {
