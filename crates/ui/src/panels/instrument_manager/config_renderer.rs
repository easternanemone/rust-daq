...
use eframe::egui;
use hardware::config::schema::{ControlPanelConfig, ControlSection, PresetValue};
...
pub fn render_config_panel(ui: &mut egui::Ui, _device: &DeviceInfo, config: &ControlPanelConfig) {
    ...
    match config.layout {
        hardware::config::schema::PanelLayout::Vertical => {
            render_sections_vertical(ui, &config.sections);
        }
        hardware::config::schema::PanelLayout::Horizontal => {
            ui.horizontal(|ui| {
                render_sections_vertical(ui, &config.sections);
            });
        }
        hardware::config::schema::PanelLayout::Grid => {
